...
def test_business_settings_view():
    ...
    try:
        ...
        request = factory.post('/settings/business/', {
        ...
        if response.status_code == 302:  # Redirect means success
            ...
            if business_settings_obj.business_email == 'test@example.com':
                ...
                print(f"ERROR: Business email was not updated. Current value: {business_settings_obj.business_email}")
                return False
        else:
            print(f"ERROR: View did not return redirect. Status code: {response.status_code}")
            return False
