...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        product_names = [
            'Smartphone', 'Laptop', 'Tablet', 'Smart Watch', 'Bluetooth Headphones',
            'Wireless Earbuds', 'Gaming Console', 'Smart TV', 'Digital Camera', 'External Hard Drive',
            'USB Flash Drive', 'Wireless Router', 'Smart Speaker', 'Fitness Tracker', 'VR Headset',
            'Power Bank', 'Bluetooth Speaker', 'E-Reader', 'Action Camera', 'Gaming Mouse'
        ]
        
        skus = [
            'SP001', 'LP002', 'TB001', 'SW001', 'BH001',
            'WE001', 'GC001', 'ST001', 'DC001', 'EH001',
            'UF001', 'WR001', 'SS001', 'FT001', 'VR001',
            'PB001', 'BS001', 'ER001', 'AC001', 'GM001'
        ]
        ...
        for i in range(20):
            ...
            try:
                ...
                product.save()
                self.stdout.write(f'Created product: {product.name} - SKU: {product.sku}, Quantity: {product.quantity}, Price: ${product.selling_price}')

        # Show total products
        products = Product.objects.filter(business=business)
        self.stdout.write(f'Total products for {business.company_name}: {products.count()}')

        # List all products
        self.stdout.write('All products:')
        for product in products:
            self.stdout.write(f'- {product.name}: {product.quantity} units, ${product.selling_price}')