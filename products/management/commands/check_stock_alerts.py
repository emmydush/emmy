...
from products.stock_monitoring import check_low_stock_alerts, check_abnormal_reduction, check_expired_products
import logging
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'[{timezone.now().strftime("%Y-%m-%d %H:%M:%S")}] Starting stock alert checks...')
        )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'[{timezone.now().strftime("%Y-%m-%d %H:%M:%S")}] Stock alert checks completed successfully!')
            )
            
        except Exception as e:
            logger.error(f"Error during stock alert checks: {e}")
            self.stdout.write(
                self.style.ERROR(f'Error during stock alert checks: {e}')
            )