...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(
                    "Backup settings not configured. Skipping scheduled backup."
                )
            )
            return
        ...
        if not backup_settings.is_active:
            self.stdout.write(
                self.style.WARNING(
                    "Automated backups are disabled. Skipping scheduled backup."
                )
            )
            return
        ...
        if (
            backup_settings.frequency == "daily"
            and current_time == backup_settings.backup_time
        ):
            should_run = True
            backup_type = "daily"
        elif (
            backup_settings.frequency == "weekly"
            and current_time == backup_settings.backup_time
        ):
            ...
            backup_settings.frequency == "monthly"
            and current_time == backup_settings.backup_time
        ):
        ...
        if not should_run:
            self.stdout.write(
                self.style.WARNING(
                    f"Not time to run {backup_settings.frequency} backup. Current time: {current_time}"
                )
            )
            return
    ...
    def cleanup_old_backups(self, retention_days):
            ...
            if filename.endswith(".zip"):
                ...
                if file_modified < cutoff_date:
                    try:
                        ...
                        self.stdout.write(
                            self.style.ERROR(
                                f"Failed to delete old backup {filename}: {str(e)}"
                            )
                        )
        ...
        if deleted_count > 0:
            ...
            self.stdout.write(self.style.SUCCESS("No old backups to clean up"))
