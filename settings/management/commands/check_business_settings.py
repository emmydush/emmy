...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--business-id', type=int, help='Specific business ID to check')
    ...
    def handle(self, *args, **options):
        ...
        for business in businesses:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'  ✓ Business settings found (ID: {business_settings.id})')
                )
                ...
                if fix_issues:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f'  ✓ Created business settings (ID: {business_settings.id})')
                    )
            ...
            if business.owner:
                ...
                try:
                    ...
                    if user_permission.can_access_settings:
                        ...
                        self.stdout.write(
                            self.style.WARNING(f'  ⚠ Owner does not have settings access')
                        )
                        if fix_issues:
                            user_permission.can_access_settings = True
                            user_permission.save()
                            self.stdout.write(
                                self.style.SUCCESS(f'  ✓ Granted settings access to owner')
                            )
                    ...
                    if fix_issues:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(f'  ✓ Created UserPermission with settings access')
                        )
            else:
                self.stdout.write(
                    self.style.WARNING(f'  ⚠ Business has no owner')
                )
