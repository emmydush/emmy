...
class Command(BaseCommand):
    help = 'Ensure all business owners have proper permissions to access business settings'

    def handle(self, *args, **options):
        # Get all businesses that have an owner
        businesses_with_owner = Business.objects.exclude(owner=None)
        
        self.stdout.write(f"Found {businesses_with_owner.count()} businesses with owners")
