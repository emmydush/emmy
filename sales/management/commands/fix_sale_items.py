...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if sale.items.count() > 0:
            self.stdout.write(
                self.style.WARNING(
                    f"Sale already has {sale.items.count()} items. Not modifying."
                )
            )
            return
        ...
        self.stdout.write(
            f"Created sale item: {sale_item.product.name} x {sale_item.quantity}"
        )
        ...
        expected_profit = (
            Decimal(str(unit_price)) - Decimal(str(product.cost_price))
        ) * Decimal(str(quantity))
        actual_profit = sale.total_profit

        self.stdout.write(self.style.SUCCESS(f"Sale total: ${sale.total_amount}"))
        self.stdout.write(
            self.style.SUCCESS(f"Expected profit: ${expected_profit:.2f}")
        )
        self.stdout.write(
            self.style.SUCCESS(f"Actual profit from property: ${actual_profit:.2f}")
        )
