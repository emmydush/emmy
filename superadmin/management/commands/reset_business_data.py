...
class Command(BaseCommand):
    help = (
        "Delete all business-related data while preserving admin users and superusers"
    )
        ...
        try:
            ...
            try:
                with connection.cursor() as cursor:
                    ...
                    cursor.execute(
                        "DELETE FROM accounting_account WHERE business_id IS NOT NULL"
                    )
                    cursor.execute(
                        "DELETE FROM accounting_transaction WHERE business_id IS NOT NULL"
                    )
                    cursor.execute(
                        "DELETE FROM accounting_journalentry WHERE business_id IS NOT NULL"
                    )
                    cursor.execute(
                        "DELETE FROM accounting_financialreport WHERE business_id IS NOT NULL"
                    )
                    cursor.execute(
                        "DELETE FROM accounting_transactionentry WHERE business_id IS NOT NULL"
                    )
            except Exception:
