...
class Command(BaseCommand):
    help = "Add sample data for kubwimana user: 150 products, 50 customers, 10 sales"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Created/verified {len(products)} products")
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Created/verified {len(customers)} customers")
        )
        ...
        for i in range(10):
            ...
            self.stdout.write(
                f"Created sale #{i+1} with {len(selected_products)} items"
            )
