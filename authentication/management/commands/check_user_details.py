    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for business in owned_businesses:
                self.stdout.write(f'  - {business.company_name} (status: {business.status})')
            ...
            try:
                ...
                self.stdout.write(f'  - can_manage_users: {user_permission.can_manage_users}')
                self.stdout.write(f'  - can_access_products: {user_permission.can_access_products}')
                self.stdout.write(f'  - can_access_sales: {user_permission.can_access_sales}')
                self.stdout.write(f'  - can_access_purchases: {user_permission.can_access_purchases}')
                self.stdout.write(f'  - can_access_customers: {user_permission.can_access_customers}')
                self.stdout.write(f'  - can_access_suppliers: {user_permission.can_access_suppliers}')
                self.stdout.write(f'  - can_access_expenses: {user_permission.can_access_expenses}')
                self.stdout.write(f'  - can_access_reports: {user_permission.can_access_reports}')
                self.stdout.write(f'  - can_access_settings: {user_permission.can_access_settings}')
