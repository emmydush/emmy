...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully fixed role and permissions for user: {user.username}"
                )
            )

        except User.DoesNotExist:
            self.stdout.write(
                self.style.ERROR(f'User with username "royke" does not exist')
            )
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f"Error fixing role and permissions: {str(e)}")
            )
