...
class POSSystem {
    constructor() {
        ...
        this.lastScanTime = 0;
        this.scanDebounceTime = 1000; // Reduced from 2000ms to 1000ms for better responsiveness
        ...
        // Log initialization
        console.log('POSSystem initialized');
    }
    ...
    initializeEventListeners() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (e.target.id === 'scanButton') {
                ...
                // Check if this event is already being handled by another system
                if (e.cartSystemHandled) {
                    console.log('Event already handled by another system');
                    return;
                }
    ...
    initializeKeyboardShortcuts() {
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (e.ctrlKey && e.key === 'b') {
                console.log('Ctrl+B pressed, opening scanner');
                e.preventDefault();
    ...
    // Create and show scanner modal - SIMPLIFIED VERSION
    createAndShowScanner() {
        console.log('=== CREATING AND SHOWING SCANNER MODAL (SIMPLIFIED) ===');
        
        // Remove any existing scanner modal
        const existingModal = document.getElementById('dynamicScannerModal');
        if (existingModal) {
            console.log('Removing existing scanner modal');
            existingModal.remove();
        }
        
        // Create scanner modal HTML - MINIMAL VERSION
        const scannerModalHtml = `
            <div class="modal fade" id="dynamicScannerModal" tabindex="-1">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div id="dynamicScannerContainer" style="position: relative; width: 100%; height: 300px;">
                                <video id="dynamicScannerVideo" style="width: 100%; height: 100%;"></video>
                            </div>
                            <div id="dynamicScannerStatus" class="mt-2 alert alert-info">Initializing scanner...</div>
                        </div>
    ...
    // Initialize the dynamic barcode scanner - COMPLETELY REWRITTEN SIMPLIFIED VERSION
    initDynamicScanner() {
        console.log('=== INITIALIZING DYNAMIC SCANNER (SIMPLIFIED) ===');
        ...
        console.log('QuaggaJS is loaded');
        ...
        console.log('Scanner elements found:', {scannerVideo: !!scannerVideo, scannerStatus: !!scannerStatus});
        
        if (!scannerVideo || !scannerStatus) {
            console.error('Scanner elements not found');
            this.showNotification('Scanner failed to initialize', 'error');
            return;
        }
        ...
        // Simple configuration like product form
        const config = {
            inputStream: {
                name: "Live",
                type: "LiveStream",
        ...
        Quagga.init(config, (err) => {
            if (err) {
                ...
                scannerStatus.textContent = 'Error: ' + (err.message || err);
            ...
            // Register callback for detected barcodes - SIMPLIFIED VERSION
            Quagga.onDetected((result) => {
                if (result && result.codeResult && result.codeResult.code) {
                    ...
                    // Process immediately
                    this.processScannedBarcodeSimple(code);
                }
            });
            
            console.log('Quagga onDetected handler registered');
        });
    }

    // Simplified barcode processing - DIRECT APPROACH
    processScannedBarcodeSimple(barcode) {
        console.log('=== PROCESSING BARCODE (SIMPLIFIED) ===', barcode);
        
        const scannerStatus = document.getElementById('dynamicScannerStatus');
        if (scannerStatus) {
            scannerStatus.textContent = `Processing: ${barcode}...`;
            scannerStatus.className = 'mt-2 alert alert-info';
        }
        
        // Direct AJAX call without wrapper functions
        fetch(`/sales/product/barcode/${barcode}/`)
            ...
            .then(data => {
                ...
                // Success - add to cart directly
                console.log('Adding product to cart:', data);
                
                // Check if product already exists in cart
                const existingItemIndex = this.cart.findIndex(item => item.id == data.id && !item.is_variant);
                ...
                if (scannerStatus) {
                    scannerStatus.textContent = `Added: ${data.name}`;
                    scannerStatus.className = 'mt-2 alert alert-success';
                }
            ...
            .catch(error => {
                ...
                // Close scanner after delay even on error
                setTimeout(() => {
                    const scannerModal = document.getElementById('dynamicScannerModal');
                    if (scannerModal) {
                        const modal = bootstrap.Modal.getInstance(scannerModal);
    ...
    // New method to manually stop scanner
    manuallyStopScanner() {
        console.log('Manually stopping scanner');
